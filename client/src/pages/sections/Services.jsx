...
export default function Services() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try { await saveSection(profile.accountId, 'services', services, sha); }
    catch (err) { setError(err.message); }
        ...
        {current ? (
          <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-5 max-w-2xl">
            ...
            <div>
              ...
              <textarea value={current.excerpt || ''} onChange={(e) => update('excerpt', e.target.value)} rows={3}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Content</label>
              <textarea value={current.content || ''} onChange={(e) => update('content', e.target.value)} rows={10}
