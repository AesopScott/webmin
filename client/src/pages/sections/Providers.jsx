...
export default function Providers() {
  ...
  useEffect(() => {
    fetchSection('providers')
      .then(({ content, sha }) => { setProviders(content); setSha(sha); })
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await saveSection('providers', providers, sha);
    } catch (err) {
      ...
      <div className="w-72 shrink-0 flex flex-col gap-3">
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden flex-1 overflow-y-auto">
          {filtered.map((p, i) => (
            <button
              key={p.id}
              onClick={() => setSelected(providers.indexOf(p))}
              className={`w-full text-left px-4 py-3 border-b border-gray-100 last:border-0 text-sm transition-colors ${
                selected === providers.indexOf(p) ? 'bg-blue-50 text-blue-700' : 'hover:bg-gray-50 text-gray-800'
              }`}
            >
